...
def render_env(slug: str, ports: dict[str, int], stack_id: str) -> str:
    ...
    lines = [
        ...
        "# --env-file) and the config loader (after .env). Process values may",
        "# override location/endpoints, never MM_STACK_NAME or MM_STACK_ID.",
        "# Stack keys only: every reader refuses any other key",
...
def _declared_identity(worktree: Path) -> tuple[str, str]:
    ...
    if not worktree.is_dir():
        raise StackError(f"{worktree}: checkout directory does not exist")
    env_file = worktree / ENV_FILENAME
    git_marker = worktree / ".git"
    if git_marker.is_file() and not env_file.is_file():
        raise StackError(
            f"{worktree}: linked checkout requires its own {ENV_FILENAME}"
            " ownership record; run 'make worktree-provision'"
        )
    if git_marker.is_dir() and env_file.exists():
        raise StackError(
            f"{worktree}: the main checkout must not carry {ENV_FILENAME};"
            " it owns the main stack (meetingminer)"
        )
    if env_file.is_file():
        values = validate_env_file(env_file, worktree.name)

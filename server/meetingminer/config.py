...
is read after it and overrides it. The process environment then overrides
location and endpoint values, but never the ownership identity
(``MM_STACK_NAME`` / ``MM_STACK_ID``) — see :func:`merged_env`. The ``docs/``
...
class Settings(_StrictModel):
    ...
    def _catalog_providers_are_declared(self) -> "Settings":
        ...
        for role in type(roles).model_fields:
            ...
            # `LlmRoles` holds only role bindings today; the guard keeps a
            # future non-binding field from turning this into an AttributeError
            # during config load.
...
def merged_env(env_path: Path) -> dict[str, str]:
    ...
    main checkout's stack. The process environment wins over both for location
    and endpoint values, except that a blank value never masks a real file
    value (finding 15: a stray `export VAR=` in a shell profile must not blank
    out a secret). ``MM_STACK_NAME`` and ``MM_STACK_ID`` are the exception:
    they come only from the ownership record (or the main-checkout defaults)
    and cannot be process-overridden. docker compose is given the same two
    files in the same order; the Makefile enforces the same identity rule.
    """
    merged: dict[str, str] = {}
    if env_path.is_file():
        ...
        misplaced = [
            key
            for key in (STACK_NAME_KEY, STACK_ID_KEY, *STACK_PORT_KEYS)
            if key in base
        ]
        if misplaced:
            raise ConfigError(
        ...
        merged.update(base)
    worktree = validated_worktree_env(env_path)
    merged.update(worktree)
    for key, value in os.environ.items():
        if key in (STACK_NAME_KEY, STACK_ID_KEY):
            continue
        if value.strip() or key not in merged:
            merged[key] = value
    merged[STACK_NAME_KEY] = worktree.get(STACK_NAME_KEY, "meetingminer")
    merged[STACK_ID_KEY] = worktree.get(STACK_ID_KEY, "")
    return merged

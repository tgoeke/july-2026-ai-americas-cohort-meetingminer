...
variables carry only secrets and the two root locations. Bindings cannot scatter
across env vars, code defaults, and flags, and the eval harness snapshots the
resolved config into every run so any run is reproducible.

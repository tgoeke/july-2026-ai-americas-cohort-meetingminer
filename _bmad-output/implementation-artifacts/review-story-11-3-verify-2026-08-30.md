...
- Resolution: Open — add a deterministic caller-level red test with a sibling probe visible during initial discovery and gone after lock acquisition; then make eligibility defer probe-marked rows to the locked refresh while preserving the refusal for genuine subject rows and stranded probes.

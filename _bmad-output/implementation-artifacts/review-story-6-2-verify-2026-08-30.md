...
Location: `server/meetingminer/youtube.py:572` · Severity: high · Finding: `validate_existing_youtube_drop()` constructs `entries` with a dict comprehension keyed by `dropFilename`. Duplicate rows collapse to one key, so a manifest containing two `recording.mp4` rows can satisfy `set(entries) == set(actual)` and be POSTed as if it exactly described the finalized evidence. · Evidence: The source-drop schema intentionally leaves `provenance` open and imposes no `uniqueItems` rule; lines 572-580 compare only the collapsed key set, not the original row count or duplicate keys. · Resolution: Open pending a red-first duplicate-row regression and a fail-closed uniqueness check.

### VF5 — The recorded existing-drop source-identity check is normally unreachable

Location: `server/meetingminer/youtube.py:648` · Severity: high · Finding: The post-build report says the local `exists` path validates source identity without yt-dlp, but `find_existing_drop()` filters candidates by `metadata.sourceId` before returning them. A directory whose name carries the requested source digest but whose metadata carries another source ID is silently treated as absent, so `validate_existing_youtube_drop()` never reaches its source-ID refusal and acquisition proceeds toward the media network. · Evidence: `server/meetingminer/mintdrop.py:482-493` skips a readable digest-matching directory when `metadata.get("sourceId") != source_id`; only returned matches reach `validate_existing_youtube_drop()` at `youtube.py:650`. The source-ID guard at `youtube.py:525` therefore covers only a narrow race where metadata changes between the two reads. · Resolution: Open pending a red-first corrupt-identity short-circuit test and a YouTube-specific fail-closed candidate check before any yt-dlp call.

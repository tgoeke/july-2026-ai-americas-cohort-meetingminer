...
Location: `server/meetingminer/youtube.py:316` · Severity: high · Finding: The shared probe/download validator treats every `vcodec` value except `None` and literal `"none"` as proof of a video stream. Empty strings, whitespace, booleans, and other malformed values therefore pass the pre-download refusal despite the remediation's fail-closed metadata contract. · Evidence: `has_video` currently tests only `entry.get("vcodec") not in (None, "none")`; unlike duration, channel, identity, and format ID, it imposes neither a string type nor non-blank normalization. · Resolution: Open pending a red-first malformed-codec matrix and a strict non-blank string check applied at both probe and downloaded boundaries.

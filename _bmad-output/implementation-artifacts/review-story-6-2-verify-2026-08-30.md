...
Location: `server/tests/test_youtube.py:678` · Severity: high · Finding: The recorded post-build resolution also claims configured-cap and started-at provenance checks for existing drops, but none of the existing-path regressions exercises those branches. · Evidence: Exact mutation `if duration > max_duration_minutes * 60:` → `if False:` in the existing-drop validator left all five existing-path regressions green (`5 passed`). After restoration, exact mutation `if provenance.get("startedAtSource") != expected_start_source:` → `if False:` also left the same five green (`5 passed`). · Resolution: Open pending red-first regressions for both branches and green restoration evidence.

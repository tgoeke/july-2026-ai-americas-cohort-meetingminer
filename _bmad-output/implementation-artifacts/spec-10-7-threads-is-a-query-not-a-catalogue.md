...
warnings: []
deferred:
  - 'B-57 — the generated TypeScript client does not know the trace operations'
  - 'B-58 — no test covers a trace stop that actually carries screens'
  - 'B-59 — an undated meeting cannot arise, so the unplaceable lane is unbuilt'
---
...
| Undated named as unplaceable | **Not built — see below and B-59.** |

## Not built here

**The undated lane (B-59).** The criteria require a meeting carrying no date to
...
**A trace stop with screens is untested (B-58).** The absent case — the one
...
**The generated client (B-57).** `make client` needs a running api and the

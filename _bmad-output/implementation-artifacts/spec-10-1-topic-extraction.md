...
- `server/meetingminer/migrations/0014_topics.sql` — NEW. `topic` (id uuidv7 PK, meeting_id FK CASCADE, name/gist NOT NULL, provenance jsonb, timestamps + `set_updated_at` trigger, index on meeting_id) and `topic_mention` (PK (topic_id, moment_id); topic/meeting composite FK CASCADE; composite FK `(moment_id, meeting_id) REFERENCES moment (id, meeting_id)` ON DELETE CASCADE — same-meeting integrity via 0009's composite key; navigation metadata, so cascade unlike `artifact`; `anchor_ms` bigint ≥ 0; index on moment_id). An AFTER DELETE trigger on `topic_mention` locks the owning topic and deletes it when no mention survives, so moment cascades and concurrent edge removal cannot leave navigation-to-nowhere rows even when extract remains settled. Comments: worker-owned, machine-derived, labelled as such; not artifacts; never in the lifecycle; replaced on rerun. Plus: widen `extraction_source_kind_check` to `('arch-summary','action-items','topics')` (drop + re-add named constraint).
...
- 2026-08-30 (owner decision, review finding #9): **delete an orphan topic.** One database-level invariant now applies across stage boundaries: after any `topic_mention` deletion, the owning topic is locked and deleted if it has no remaining mention. This covers moment FK cascades during augmentation while `extract` remains settled and serializes concurrent last-edge removals. Replaced the migration test that accepted a mention-less topic, added the two-mention preservation case, and added an integration regression that re-arms only `moments`, removes the sole screen-derived mentioned moment, proves `extract` did not rerun, and requires the topic to disappear. Both last-mention tests were observed red before the trigger and green afterward.
...
- A topic with no surviving mention does not exist (owner decision, review finding #9). The database deletes it after the final mention disappears, including through a moment cascade while augmentation leaves extract settled; a topic with any surviving mention remains.

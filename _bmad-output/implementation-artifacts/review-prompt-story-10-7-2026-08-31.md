...
- **B-59 — the undated lane is unbuilt.** The criteria require an undated
...
- **B-58 — no test covers a stop that actually carries screens.** Seeding a
  `screenshot` needs the whole `screen` chain. The absent case, which is the one
  AD-18 turns on, is covered on both sides.
- **B-57 — the generated TypeScript client does not know the two new
...
- `web/src/features/threads/`: `trace.ts`, `traceApi.ts`, `TraceTimeline.tsx`,
  ...
  files repointed
- `docs/backlog.md` (appended: B-57, B-58, B-59)

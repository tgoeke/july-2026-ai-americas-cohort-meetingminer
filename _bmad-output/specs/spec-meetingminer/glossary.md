...
**⚠ collision — "Artifact"** — two unrelated meanings in this repo:
1. **Extracted artifact**: an ADR, action item, decision, requirement or similar, extracted by the
   pipeline from a moment and subject to the publish gate.
2. **Planning artifact**: a document under `_bmad-output/planning-artifacts/` or
   `_bmad-output/implementation-artifacts/` (epics, story specs, reviews).
   Meaning (1) is the product; meaning (2) is the process. Neither is going to be renamed, so read
   the path.
...
**⚠ collision — "Spec"** — two meanings:
1. **`SPEC.md`**: the five-field kernel (Why, Capabilities, Constraints, Non-goals, Success signal)
   plus its companions — the canonical contract for what to build.
2. **A story spec**: `_bmad-output/implementation-artifacts/spec-1-13-*.md`, one story's
   implementation plan.

**Companion** — a file listed in `SPEC.md`'s `companions:` frontmatter that downstream must read
alongside it. Distinct from `sources:`, which are inputs fully absorbed into the spec and which

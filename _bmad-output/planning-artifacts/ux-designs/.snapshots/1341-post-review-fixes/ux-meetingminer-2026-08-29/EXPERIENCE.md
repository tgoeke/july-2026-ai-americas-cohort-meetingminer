...
| Beyond palette | Threads | Bands 25+ in grey with a grey swatch; the list item says `beyond the palette (12 hues × 2 laps) — identified by name`. |
...
| Shape-invalid URL | Add-meeting | Under the field, muted: `Not a YouTube video URL — paste a watch or youtu.be link.` Submit stays disabled. Not a refusal box (nothing was refused; nothing was sent). A playlist URL is refused here too: `Playlist URLs are not accepted on this tab — paste one video's watch link.` (story 6.2a acquires playlists from the command line; no UI story owns them — `findings-for-epics.md`). |
...
| Probe refused | Add-meeting | A refusal box under the field with the rule the tool would refuse on — `youtube-drop: duration over cap`, `…: private or removed video`, `…: no video stream`, `…: not a YouTube video` — detail and remediation; Submit disabled. The probe endpoint is a gap against story 6.4 (`findings-for-epics.md` F-6); until it exists the same refusal arrives after launch (Tool refusal, below). |
...
| File rows: name, size, classification | client (`File` API: recording, speaker-bearing `.txt`, `.vtt`; the `Name: text` cue sniff is a hint only) | story 6.5a |
...
4. Enter fills the field with the suggestion and closes the list; Enter again (or Save) sends `PUT /meetings/8f3c…/speakers/SPEAKER_00`, which returns 200; the row shows `Priya Natarajan` and the state word `queued` under `rerun`; three stage bars — `align`, `moments`, `extract` — appear in the header and start filling.

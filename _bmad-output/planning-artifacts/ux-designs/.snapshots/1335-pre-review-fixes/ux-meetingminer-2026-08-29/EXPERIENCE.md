...
| Title, date, corpus `real` for uploads | drop `metadata.json` requirements (`[ASSUMPTION]` the upload session accepts `title` and `startedAt`; the dialect is declared per story 6.4a) | story 6.4a · AD-1 |

...
colors:
  ...
  # Moment kinds — three hue families, told apart within a family by glyph: records (decision, adr) at 290°, actions (action-item) at 55°, backlog (story, requirement) at 190°, corrections (bug-fix, change-request) at 345°. Chip = fill + text + border; the same text color is the kind used as plain colored text.
...
Everything else — chrome, text, borders, inputs, screenshots — sits on a warm dark grey that replaces `web/src/index.css`'s neutrals by six token values, and the one non-semantic hue, ember, marks the primary action and focus and nothing else. The result reads like a dark instrument panel with warm paper behind the glass, where the colored elements are the readings.
...
Eight hues, 45° apart, at L 0.75 C 0.12 — fewer than the twelve of the other systems because a warm base flatters fewer, wider-spaced hues, and eight is what a reader tells apart at a glance; they sit on the page at ≥ 8.36:1 as a band and read as text on a card at ≥ 7.54:1 (all eight AAA). A thread's hue is **stable**: `[ASSUMPTION]` it is assigned client-side by the thread's ordinal when threads are ordered by first mention (`firstMentionAt`, then `threadId`): ordinal mod 8 is the hue, ordinal div 8 is the lap. A thread keeps its color across sessions, sorts, and filters without the api knowing about palettes; a new thread never recolors an old one. A merge keeps the survivor's ordinal and retires the merged thread's (ordinals are never reused); a split gives the new thread the next ordinal. Two adjacent bands in the Threads view can share a hue when the view is sorted by activity; the design accepts that because the band's `#name` label and its lap swatch are always rendered.
...
| Assign thread hues by first-mention ordinal (mod 8) and always print the name with its lap swatch | Assign hues by list position, or identify a thread by color alone |

...
Desktop-first web, single surface, dark only. React 19 + react-router 7 + Vite, shadcn/ui components on Tailwind v4 with Base UI primitives (`components/ui/button.tsx`), Geist Variable. `DESIGN.md` is the visual identity and states the delta from `web/src/index.css`; this spine specifies only the behavioral delta from shadcn's defaults.

Viewport numbers live in `DESIGN.md` · Layout & Spacing. The full-density composition targets 1280 × 800 and gains a third Moments column at 1440; a narrow/reflow presentation at ≤ 900 CSS px preserves every operation in one column for WCAG 1.4.4 and 1.4.10. It is not touch-first: pinch-zoom on the timeline is supported because trackpads emit it, not because a phone-only gesture is required.
...
| Moment card | Moments | The screenshot and the title are one link to the moment view, named by the title. **Replay** (`aria-expanded`) toggles the inline player under the screenshot; the card expands to span the grid; at most one card is expanded (opening another collapses the first — the single-open-key pattern `MeetingMoments` already uses); on collapse — `Esc`, another Replay, or Open moment — focus returns to that card's Replay button. **Open moment** → `/moments/:id`. **Open on YouTube at H:MM:SS** is present only when `sourceDeepLink` is a YouTube URL; it parses the link with `URL`, replaces or inserts the provider's time parameter from `startMs`, and opens the result in a new tab. It never concatenates `&t=` onto an unparsed string. Kind chips filter the feed by that kind on click; thread chips open `/threads/:threadId`. The reason line is read-only. |
| Reason line | Moment card | Renders the non-empty `reasons[]` from the feed in api order with `label` verbatim. Artifact kinds → kind chip; `due`, `risk`, `question`, `recency`, and `published` → plain muted text with a literal prefix; `thread` → thread chip. No reason is composed client-side. Story 10.4 validates reasons before pagination, drops and logs an item with an empty or unknown reason, and computes `items`, `total`, and offsets only from remaining serializable rows. The client treats an invalid item that escapes as one page-level contract error rather than silently changing counts. `ranked by recency` appears only when the api returns it. |
...
| Thread chip | cards, reasons, timeline, moment view | `#name`, accessible name `thread <name>` (the `#` is hidden); opens the thread in Threads focused at the meetings tier around the current moment. Hue and lap come only from the persisted immutable `colorOrdinal` (`DESIGN.md` · Threads), never current list position. |
...
| Clip button | Speaker naming | Three per tag, at the api's sample offsets; press plays that clip in the one player on the panel from `offset` to optional `endMs = startMs + 8000`, then pauses (story 7.4). Existing player callers omit `endMs` and retain open-ended playback. Keyboard `1` `2` `3` play clip n of the selected tag. |
...
| Model select | ask box, Settings | Trigger shows `role · binding model id · provider ● state`; its accessible name includes the health word and the `●` is hidden. Popover is a focusable single-select `listbox` with `aria-activedescendant` and a named `group` per provider; each catalog option joins to `GET /status.providers[]` by exact provider id and carries that provider's health dot + word. The active binding is `aria-selected` (drawn as ✓); an option whose provider key is invalid or missing is not `aria-disabled` and carries its remediation as `aria-description`; selecting calls `PUT /settings/roles/{role}` and, on success, updates the trigger — the next question uses the new binding. An option whose provider key is `invalid`/`missing` stays selectable and is rendered muted with its remediation, because choosing it must fail *loudly* at the ask, not silently be filtered out. Rapid selections use the asynchronous ownership rule below, so an older response cannot overwrite the latest choice. Keyboard: arrow keys move, Enter selects, Esc closes. |
...
| Probe running | Add-meeting | 600 ms after the shape check passes, `POST /acquisitions/probe` checks the normalized URL without writing. Under the field, muted mono: `Probing…`; Submit disabled. Editing the URL aborts or supersedes the request under the asynchronous ownership rule. |
...
| Probe refused | Add-meeting | A refusal box under the field with the rule the tool would refuse on — `youtube-drop: duration over cap`, `…: private or removed video`, `…: no video stream`, `…: not a YouTube video` — detail and remediation from Problem Details; Submit disabled. |
...
| Already ingested | Add-meeting | Status is **posted** with `result: exists`, the existing ids, `Already in the corpus — nothing downloaded.`, and the existing meeting card. |
...
| Catalog empty | Model select | Trigger remains visible but cannot open an empty choice list; beneath it: `No models configured — add a binding in config.yaml and restart the api.` No default or substitute is invented. |
...
**Asynchronous ownership.** Probe, timeline-tier fetch, and model-selection requests each carry a monotonically increasing generation plus the normalized input key that started them. Starting newer work aborts older work when the transport allows it. A response may update visible state only when both generation and key still equal the control's current state; late success and late failure are discarded. Loading indicators, `aria-busy`, focus transfer, and Retry belong to the current generation only.

## Semantic Zoom

Only the Threads timeline animates (model: continuous zoom as in Google Earth — owner direction), and only it fetches by what is visible (story 10.3). The canvas maps the api's canonical UTC `occurredAt` to x: `x = (occurredAt − windowFrom) / scale`, with `scale` in **milliseconds per pixel**. `startMs` remains the replay offset; it never determines cross-meeting x on the client. Every tier uses the same mapping, so an item's x is unchanged across a tier change. If a meeting has day precision, the server anchors it at `00:00:00Z + startMs`, preserves `occurredAtPrecision: day`, and sorts equal anchors by `meetingId`, then `momentId`; the client labels it with its date only.
...
| **moments** | 2 000 ≤ scale < 240 000 (2 s/px – 4 min/px) | scale < 4 min/px | scale ≥ 5 min/px | `level=moments` | moments as ticks at `occurredAt` with title and speakers-where-known, each inside a ≥ 24px hit area, overlapping ones clustered (`3 moments, 12:40–12:52`); the meeting's span drawn as a bracket; the cell is the moment or cluster |
| **evidence** | scale < 2 000 (2 s/px) | scale < 2 s/px | scale ≥ 2.5 s/px | `level=evidence` | each moment becomes a `{components.lod-card}` anchored at `occurredAt`: screenshot, transcript excerpt, artifact kind chips, replay offset; the cell is the card |
...
**Focus across tiers.** Focus is never lost to the page. On entering the grid, focus lands on the cell nearest the window center on the focused (or first) thread. On any tier change — wheel, keys, `Enter`, `Backspace`, double-click — focus moves to the cell in the new tier whose span contains the previous cell's `x`: `Enter` lands on the first child cell, `Backspace` on the parent cell. If the new tier has no containing or nearest cell, focus lands on the grid root and the polite region announces `No moments in view.` The grid's accessible name carries the tier and window (`Threads timeline — bands tier, 2026-03-01 to 2026-08-29`) and the polite region announces the change once.

**Fetch discipline.** The window fetched is the visible window padded 50% each side at the current tier; requests are debounced 120 ms and cached by `(sortedPinnedThreadIds, level, bucketedFrom, bucketedTo)`, so pin membership is part of identity even when tier and window do not change. Each request obeys Asynchronous ownership: a response for an older window/tier generation is discarded even if it arrives last. A finer tier is never drawn from a coarser tier's data — the outgoing tier stays until the current fetch resolves, and a failure keeps it drawn with a refusal box under the canvas. Nothing is interpolated: a bucket with no data is drawn at 0.08 alpha, a meeting without moments in the window draws its bracket and `no moments in view`.
...
Behavioral. Contrast lives in `DESIGN.md` · Colors · Contrast (every pair measured; AA minimum). Target: WCAG 2.2 AA with keyboard and screen reader. At 200% text resize and down to 320 CSS px, the narrow presentation reflows without two-dimensional page scrolling: chrome becomes two rows; Moments is one column; Threads keeps its time axis in a labeled horizontal-scroll region while the list stacks above it; Add-meeting stays one column; Speaker naming stacks speaker list, clips/naming, then transcript. Focus and DOM order remain the order stated below.
...
- **Text and reflow.** Text scales to 200%. At 320 CSS px no page-level horizontal scroll is required; the timeline itself is the allowed two-dimensional data region and is named as such, with all controls reachable outside its scrollport.

## Responsive & Platform

Desktop-first; the density and narrow/reflow numbers are in `DESIGN.md` · Layout & Spacing. Chrome on macOS is the recording target; Safari and Firefox are expected to work because nothing beyond standard DOM, `<video>` with Range, and CSS is used. No print styles. No offline mode: every screen already states `Cannot reach the api at …` in place and keeps stale content labeled.
...
- **Rejected:** rendering `risk` or `question` as publishable artifact kinds (Story 10.4 stores them as ranking signals and reasons); topic chips as facts (FR41); auto-applied speaker suggestions; silent model fallback; decorative color; skeleton cards; toasts; modals. Each is an invention or a hiding place.

## Data Traceability

Every element on every screen, the field that backs it, and whether it exists today (`existing`, from `web/src/client/types.gen.ts` at e5510c7) or is created by a story in this plan (`story N.N`, from that story's acceptance criteria in `epics.md`). The owning story contracts were enumerated by the 2026-08-29 review decisions; no row relies on an inferred field. Nothing decorative survives this table.
...
| Health per model option | `GET /status` → `providers[]{provider,keyState,detail,remediation}`, joined to each catalog option by exact provider id; active binding remains in the role model response | story 8.2 · story 8.2a |
...
| Header count `Moments 24` | response `total`; visible count from `items.length` | story 10.4 |
| Reason line | non-empty item `reasons[]{kind,label,ref?,at?}` rendered verbatim; risk/question are persisted ranking-signal reasons, not artifacts | story 10.4 |
| Kind chips in reasons | reason `kind` ∈ `MomentArtifact.kind` (existing enum); `due | risk | question | recency | published` render as text | story 10.4 |
| Thread chips | item `threads[]{threadId,name,colorOrdinal}` | story 10.4 |
| Screenshot | opaque item `screenshotId` → ID-addressed `GET /media/files/{screenshotId}` | story 10.4 (route and field; AD-17) |
| Screenshot `alt` | `<viewType> at <offset>, <meetingTitle>` from item `viewType`, `startMs`, `meetingTitle` | story 10.4 |
...
| Meeting title, date, offsets, corpus | item `meetingId`, `meetingTitle`, `startedAt`, `startedAtPrecision`, `startMs`, `endMs`, `corpus` | story 10.4 |
| Excerpt | item `preview` | story 10.4 |
...
| Open on YouTube at H:MM:SS | parse item `sourceDeepLink` with `URL`; replace/insert provider time parameter from `startMs` | existing · story 6.6 |
| Empty / filter-empty sentences | feed returns zero items | story 10.4 |
| Show 24 more | response/query `limit`/`offset` | story 10.4 |
...
| Element | Backing field / operation | Source |
|---|---|---|
| Thread list: name, mention count, last mention, meetings | `GET /threads` item `threadId, name, mentionCount, meetingCount, firstMentionAt, lastMentionAt, colorOrdinal` | story 10.3 |
...
| Split panel: the thread's topics by meeting | topic membership in story 10.3's `meetings` level | story 10.2a · story 10.3 |
...
| Moments: title, absolute placement, replay offset, speakers, screenshot | `level=moments` → `momentId`, `meetingId`, `title`, `occurredAt`, `occurredAtPrecision`, `startMs`, `speakers`, `screenshotId` | story 10.3 |
...
| Replay on a LOD card | evidence item `meetingId`, `startMs`, `hasRecording`; recording route is ID-addressed `GET /media/recordings/{meetingId}` | story 10.3 · existing |
| Open moment | route `/moments/:momentId` | existing |
| Thread hue and lap | persisted immutable `colorOrdinal`; client maps ordinal 1–8 to lap 1, 9–16 to lap 2, 17+ to grey | story 10.3 |
...
| URL probe before submit: title, duration, captions, source id, or the refusing rule | `POST /acquisitions/probe` → `{title,durationMs,captions,sourceId}` or refusal Problem Details; writes nothing | story 6.4 · story 6.5 |
| Tool refusals: not a video, private/removed, no video stream, tools missing, duration over cap | `GET /acquisitions/{id}` `failed` with `refusal{rule,detail,remediation}` plus the log tail | story 6.2 (rules) · story 6.4 (status) |
| Already ingested (`exists`) | `GET /acquisitions/{id}` → `posted`, `result: exists`, existing job and meeting ids | story 6.2 · story 6.4 |
...
| Title, started-at value, corpus `real` for uploads | upload session fields `title`, RFC 3339 `startedAt` including numeric UTC offset, `corpus: real`, explicit `transcriptDialect` | story 6.4a |
...
| Provenance line on the finished card (`youtube:<videoId> · yt-dlp 2026.07.04`) | posted status `source{sourceId,tool,toolVersion}` | story 6.2 writes it · story 6.4 serves it |
...
| Resolved-by-source rows (Zoom) | speakers row nullable `participantId` and `displayName`, populated when resolved | story 7.2 (shape) · story 7.4 (correction) |
...
| Provider health per option | `GET /status` → `providers[]{provider,keyState,detail,remediation}` plus active binding per role | story 8.2a |
| Save | `PUT /settings/roles/{role}`; 422 for a binding outside the catalog | story 8.2 |
| Failing binding named at the ask | RFC 9457 `urn:meetingminer:problem:binding-failed` with provider, binding, upstream status in `detail` | story 8.2 |
...
All eight are sessions of Tim, the owner — an architect reviewing recordings alone. The full-density examples use a 1440 × 900 Chrome window; every flow also has the narrow/reflow presentation defined above.
...
2. He pastes `https://www.youtube.com/watch?v=dQw4…`. The shape check passes; after 600 ms `POST /acquisitions/probe` shows `Probing…` with Submit disabled, then returns the title, duration, captions, and source id. Submit enables; nothing has been written.
...
1. Tim pastes a URL to a four-hour conference stream. The shape check passes and the probe begins; Submit stays disabled.
2. `POST /acquisitions/probe` returns Problem Details without launching a process or writing state.
3. **Climax:** the refusal box under the URL reads `youtube-drop: duration over cap — 4h 02m exceeds the configured 180 minutes. → Raise acquisition.youtube.maxDurationMinutes in config.yaml and restart the worker, or choose a shorter video.` Nothing was downloaded, minted, or submitted; no acquisition or meeting row exists.
4. He edits the URL; the refusal box clears because its cause changed, the previous probe is superseded, and the new URL must pass its own probe before Submit enables.
...
3. He chooses `zoom`, types the title `Config catalog walkthrough` and started-at value `2026-08-18T09:00:00-06:00`; corpus reads `real`. Submit enables. Nothing has been sent and no timestamp was inferred from a date.
...
5. **Climax:** the list shows twelve threads; `judge bake-off` receives the next persisted `colorOrdinal` and its own hue; `eval harness` keeps its ordinal and hue and loses two mentions; both rows carry `curated` beside `machine-derived`. The next rerun of extraction will not undo any of it — the api-owned alias rows survive (AD-5).

...
| Reason line | Moment card | Renders the non-empty `reasons[]` from the feed in api order with `label` verbatim. Artifact kinds → kind chip; `due`, `risk`, `question`, `recency`, and `published` → plain muted text with a literal prefix; `thread` → thread chip. No reason is composed client-side. Story 10.4 drops and logs an item with an empty or unknown reason before serialization; the client treats one that escapes as a contract error and omits the card. `ranked by recency` appears only when the api returns it. |
...
| Title, date, corpus `real` for uploads | upload session fields `title`, RFC 3339 `startedAt`, `corpus: real`, explicit `transcriptDialect` | story 6.4a |
